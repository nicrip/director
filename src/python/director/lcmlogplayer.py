...
class LcmLogPlayer(object):
    ...
    def __init__(self, lcmHandle=None, eventTimeFunction=None):
        if not lcmHandle:
            lcmHandle = lcmUtils.getGlobalLCM()
        self.eventTimeFunction = eventTimeFunction
    ...
    def readLog(self, filename, progressFunction=None):
        ...
        while True:
            ...
            if self.eventTimeFunction:
                eventTimestamp = self.eventTimeFunction(event)
            else:
